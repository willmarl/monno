...
export enum CommentSearchFields {
  CONTENT = 'content',
  CREATOR_USERNAME = 'creator.username',
  RESOURCE_TYPE = 'resourceType',
}

const VALID_POST_SEARCH_FIELDS = Object.values(CommentSearchFields);
...
export class CommentSearchDto extends PaginationDto {
  ...
  getOrderBy(): Record<string, 'asc' | 'desc'> {
    ...
    const validFields = ['createdAt', 'updatedAt', 'likeCount', 'resourceType'];
  ...
  @ApiPropertyOptional({
    description:
      'Filter by deleted status. If not provided, shows both deleted and active comments.',
    example: false,
  })
  @IsOptional()
  @TransformBoolean()
