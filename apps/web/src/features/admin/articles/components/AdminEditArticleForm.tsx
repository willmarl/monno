...
interface AdminEditArticleFormProps {
  onSuccess?: () => void;
  onCancel?: () => void;
...
/**
 * Admin form for editing articles with media management.
 *
 * onSuccess handles success toast and any redirect — caller already has the ID.
 * Modal: close modal + toast. Admin page (if applicable): push to detail route.
  ...
  return (
    <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
      ...
      <div className="space-y-2">
        <Label htmlFor="admin-inline-title" className="text-sm">
          Title
        </Label>
        <Input
          id="admin-inline-title"
          type="text"
          placeholder="title"
      ...
      {/* content */}
      <div className="space-y-2">
        <Label htmlFor="admin-inline-content" className="text-sm">
          Content
        </Label>
        <Textarea
          id="admin-inline-content"
          placeholder="content"
          disabled={isSubmitting}
      ...
      {/* status */}
      <div className="space-y-2">
        <Label htmlFor="admin-inline-status" className="text-sm">
          Status
        </Label>
        <Controller
          name="status"
          ...
          render={({ field }) => (
            <Select value={field.value || ""} onValueChange={field.onChange}>
              <SelectTrigger id="admin-inline-status" disabled={isSubmitting}>
