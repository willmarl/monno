...
> **Use the image/media column snippet from `19{L}.md` only.** Insert it after the title column in the template below. Do not copy columns from other path files.
...
export const columns: ColumnDef<{{resource}}>[] = [
  ...
  // image/media column — from 19{L}.md
  {
    accessorKey: "content",
...
> After building the data table (step 2), add the admin **Create** button + modal in `22-admin-create.md` step 6. Until then, use a minimal page shell:
...
example (before create button — interim):
...
> **Final page** (with Create button): see `22-admin-create.md` step 6.
...
<!-- NEXT: 19{L}.md then 22-admin-create.md (skip 22 if !ADMIN) then 20.md (skip if no search) -->
