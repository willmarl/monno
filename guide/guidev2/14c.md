...
>
> **Cache invalidation:** Import `useQueryClient`. After `applyCreateMediaChanges`, call `await queryClient.invalidateQueries({ queryKey: ["{{resource}}"] })` (or `["articles"]`) **before** redirect. See create + media cache rule in `14.md`.
...
Adapt field names and routes for your resource. Keep the two-phase submit: text create first, then `applyCreateMediaChanges`, then invalidate list queries, then redirect (user) or `onSuccess` (admin modal).
