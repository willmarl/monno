...
> **Create + media cache rule:** When create is **two-phase** (text `POST` first, then `applyCreateMediaChanges` / media sub-routes — complex path `14c`, admin create `22-admin-create.md`):
> 1. **Do NOT** put `invalidateQueries` in `useCreate{{resource}}` / `useAdminCreate{{resource}}` `onSuccess` — the list refetches before media exists and shows "No image".
> 2. **Do** call `queryClient.invalidateQueries` in the **form** only **after** media upload finishes (or immediately after create if there were no queued files).
> 3. Admin create forms **do not redirect** — invalidate `["admin-{{resource}}"]` (or `["admin-articles"]`) so the dashboard table refreshes with media attached.
>
> Single-phase create (simple path `14b` — file in one FormData `POST`) may still invalidate in the mutation `onSuccess`.
