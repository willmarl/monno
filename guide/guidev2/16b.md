# part 16b | {{resource}} component — AppImage / imagePath

Instructions for AI: roughly guess UI component based off schema model and type.ts. Quick check that api/hooks work — doesn't matter if ugly.
...
example:
`web/src/components/ui/Article.tsx` — same pattern with `imagePath` + `AppImage`.
