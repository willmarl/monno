...
> **Two-phase create + media (complex):** Do not invalidate in `useCreate{{resource}}` `onSuccess` — invalidate in the form after media (see `14.md` cache rule). Simple path below may keep hook invalidation if file uploads in a single `POST`.
