...
Include scoped search (see `profile-search.md`): debounced `ProfileListSearch` + `query` on the by-user hook.
...
export function Users{{resource}}List({ user, isOwner }: Users{{resource}}ListProps) {
  ...
  const { data, isLoading } = use{{resource}}ByUserId(
    user.id,
    page,
    DEFAULT_LIMIT,
    query,
  ...
  return (
    <div>
      <ProfileListSearch
        placeholder={`Search {{resource}} by ${user.username}…`}
        value={query}
        onChange={(q) => {
          setQuery(q);
          setPage(1);
      ...
      <PaginatedListInline
        ...
        renderItem={(item) => <{{resource}} data={item} isOwner={isOwner} />}
        title={`{{resource}} by ${user.username}`}
        layout="grid"
        emptyMessage={
          query ? `No {{resource}} matching "${query}".` : "No {{resource}} yet."
        }
      />
    </div>
  );
...
If `ProfileListSearch.tsx` does not exist yet, create it (debounced input; local state only — not URL `q`). Full API wiring is in `profile-search.md`.
...
<!-- NEXT: profile-search.md if PROFILE else (19.md if ADMIN then 19{L}.md; else 20.md if SEARCH else 21.md) -->
